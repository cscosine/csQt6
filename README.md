...
├── qt6.py                          # Main orchestrator pipeline recipe
...
│   └── atp_packages_list.py        # System package dependencies for Ubuntu
├── csorchestrator/sdk/portable/    # Vendored portable SDK for manifest generation in CI
│   └── apt_packages_list.py        # System package dependencies for Ubuntu
├── csorchestrator/sdk/portable/    # [Autogenerated / Vendored] Standalone SDK synced from csOrchestrator
├── tests/
│   └── csQt6/                      # Unit tests for configuration mappings
└── .github/workflows/
    └── Qt6.yml                     # Auto-generated GitHub Actions CI workflow
...
| [`csorchestrator/sdk/portable/`](csorchestrator/sdk/portable/) | Autogenerated & exported by `csOrchestrator`. | Synced from `csOrchestrator` to provide lightweight release manifest utilities on CI runners without requiring `pip install csorchestrator`. |
...
- [ ] **Separate Namespace for Portable SDK**:
  - Rename the vendored portable SDK module from `csorchestrator.sdk` to `csorchestratorsdk.portable` (or `csorchestrator_sdk.portable`) to completely prevent local Python imports from shadowing the installed `csorchestrator` package.
- [ ] **Qt Module Subsetting**:
  - Support building a minimal subset of Qt modules (e.g. passing `-submodules qtbase` to `./configure`) for faster turnarounds when only core Qt libraries are needed.
- [ ] **Smoke Tests for Pipeline Construction**:
  - Add quick unit tests verifying that `create_orchestrator()` successfully builds the execution graph without raising syntax or import errors.
